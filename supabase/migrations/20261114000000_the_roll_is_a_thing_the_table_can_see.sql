-- ============================================================================
-- The roll is a thing the whole table can see
-- ============================================================================
-- `xp_arbitrate` has decided the dice since 20261101000000 and deliberately did
-- not keep it. The note there said why, and it was half right: the face is the
-- level's business, it lands in the document's own `data`, and a copy here
-- would be a second answer to "what did I roll".
--
-- The half that was wrong is which clients that covers. A level's `data` is not
-- a channel: it is written on a timer and read at open, and on the builtin
-- `/xp/<slug>` route there is no store behind it at all, because `xpStore`
-- wants a UUID. So the roller's screen updated from the reply and **nobody
-- else's ever heard about it**. Three players sat at one table watching a
-- fourth of a game happen.
--
-- Found by `e2e/mensch.e2e.ts`, which is three real accounts in three real
-- browsers - the only check in this repo that can see a thing arrive on one
-- screen and not another.
--
-- So the roll is recorded and `xp_arbiter_view` returns it, which is where the
-- turn already goes and for the identical reason: whose turn it is and what was
-- rolled are the two facts a board game draws every frame, and a table where
-- only one player knows either is not a table.
--
-- **It is still one answer.** That is what the old note was right about, and
-- nothing here weakens it: this is *the* roll, and every client's `data` is a
-- mirror rather than an opinion. `by` and `at` are what let a client use it as
-- one - a bare number cannot say whether the four on screen is this turn's or
-- the last one's.
--
