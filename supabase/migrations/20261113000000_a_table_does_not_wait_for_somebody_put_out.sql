-- ============================================================================
-- A table does not wait for somebody the room has put out
-- ----------------------------------------------------------------------------
-- 20261107000000 built the turn and took its order from `health`'s keys. That
-- map is the scoreboard and it never shrinks - a player who is eliminated stays
-- in it deliberately, because a name that vanished would read as somebody
-- leaving rather than as somebody knocked out. So the order kept handing turns
-- to people who are not allowed to take one, and the table waited for a
-- spectator.
--
-- Every other rule in this function already knows what being out means:
-- `(lives ? key) and (lives ->> key)::integer <= 0`, which `revive`, `hit` and
-- `vote` all test. The order was the one place that did not ask.
--
-- ---------------------------------------------------------------------------
-- Why this is the fix and a timeout is not
-- ---------------------------------------------------------------------------
-- docs/xp/server-authority.md §4.2 names the stall - a table stops on somebody
-- who closed their tab - and refuses to close it with a timer, because a rule
-- about how long a person may think is one nobody in the room agreed to.
--
-- Elimination is the opposite: it is a **majority decision the vote already
-- carries**. Skipping the people it named is not a new rule, it is this order
-- stopping its disagreement with an old one. The tab-closing case is still open
-- and still wants a `skip` a majority can press, which is the same machinery
-- again.
--
-- ---------------------------------------------------------------------------
-- The turn the elimination was holding
-- ---------------------------------------------------------------------------
-- The likeliest moment for a room to vote somebody out is while they are the
-- one everybody is waiting for. They cannot pass it themselves - `pass` refuses
-- a caller who is not in the order - so a turn pointing at somebody who is out
-- may be moved on by anybody, to the next player *after* them, not to the first:
-- being knocked out should not hand the lead back to the top of the table.
--
