-- ============================================================================
-- A number the whole table keeps, for exactly one game
-- ----------------------------------------------------------------------------
-- The gap docs/xp/backlog.md 3a calls the cheapest of the four and the only one
-- with a level already waiting on it: there was nowhere to put a number that
-- **everybody agrees on** and that is **gone when the game is**.
--
-- The four scopes a document can declare divide on one axis only. `player`,
-- `space` and `shared` all persist; `run` does not persist and, because the
-- only transport a level's data had was the store, it did not travel either -
-- so a `run` field was a number one client kept to itself. That left a table
-- counting its pieces home in `space`, which is the bug data.ts documents at
-- length: a finished game leaves a four in the row for ever, and the next table
-- opens already won.
--
-- ---------------------------------------------------------------------------
-- The arbiter already was the answer, for everything except a declared field
-- ---------------------------------------------------------------------------
-- Scores, lives and health are exactly "shared and fresh" and have always been
-- kept here - agreed by one writer, published to everybody, and cleared by
-- `reset`. So this adds no mechanism: a `run` field is one more number in the
-- same jsonb, cleared by the same rematch, read through the same view. No
-- table, no column, no rows to sweep up afterwards - which is what a store key
-- per run would have cost.
--
-- ---------------------------------------------------------------------------
-- What is deliberately not checked
-- ---------------------------------------------------------------------------
-- **What the number means.** The rules live in the document - what may add to a
-- counter, and what it is for - and the arbiter has never read one. This is the
-- same line `moved` draws one branch down: what it owns is that everybody hears
-- the same number, once, and that it is gone when the game is.
--
-- **Who may write it.** Any player in the match may, which is what `shared`
-- already means one scope over and what a co-operative counter needs. A level
-- that wants one writer has `turn_start` for it.
--
-- The rest of this file is 20261126000000 restated, because `create or replace`
-- takes the whole function and rebuilding one from an older copy has silently
-- dropped a branch in this file's siblings before.
-- ============================================================================
...
comment on function public.xp_arbitrate(text, text, jsonb) is
  ...
  'field, turn_start / pass, and vote_open / vote / vote_close. Returns an XpVerdict as '
...
comment on function public.xp_arbiter_view(text) is
  ...
  'round, the level s own run fields, whose turn it is and who may be drawn how '
  'are public - a look, never '
  'a role. The only read path to xp_arbiter_state.';
