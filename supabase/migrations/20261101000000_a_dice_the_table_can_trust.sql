-- ============================================================================
-- A dice the table can trust
-- ----------------------------------------------------------------------------
-- The eighth rule, and the second one that exists because a number decided by a
-- client is not a number: `deal` is here so a role cannot be re-rolled, and this
-- is here so a *face* cannot. `packages/xp/src/random.ts` is a seeded stream
-- every client reproduces on purpose - that is right for scenery, where
-- everybody agreeing matters more than nobody predicting, and exactly wrong for
-- a dice.
--
-- ---------------------------------------------------------------------------
-- The face is returned and not kept
-- ---------------------------------------------------------------------------
-- Every other rule here writes into `xp_arbiter_state`, and this one does not.
-- What a dice came up as is the *level's* business: the `roll` verb names a
-- field in the document's own `data` block and the host writes it there, which
-- is where a rule reads it back. A second copy in this state would be a second
-- answer to "what did I roll", and the two would disagree the first time
-- somebody rolled twice in one round.
--
-- ---------------------------------------------------------------------------
-- What it does not do, said plainly
-- ---------------------------------------------------------------------------
-- **It does not know whose turn it is.** Any player in the match may roll, as
-- often as they like. The arbiter has never been told about turns - the same
-- gap the note on `reset` in docs/xp/server-authority.md describes - and a
-- refusal that cannot name whose turn it is would be a guess wearing a rule's
-- clothes. A board game built on this today enforces its turn order in the
-- document, which is a rule between friends rather than a rule the table keeps.
--
-- The whole function is restated, as every migration that touches it has been:
-- `create or replace` takes the body entire, and rebuilding it from an older
-- copy has silently dropped a branch here before.
-- ============================================================================
