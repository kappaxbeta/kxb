-- ============================================================================
-- A rematch the arbiter knows about
-- ----------------------------------------------------------------------------
-- Pressing `R` rebuilt the level, reset the clock and put everybody back at the
-- spawn - and left every kill, every life and every elimination exactly where
-- they were. So the second round opened with the first round's scoreboard, and
-- anybody knocked out of round one was knocked out of the rematch as well, for
-- good, with no way back short of everybody leaving the room.
--
-- ---------------------------------------------------------------------------
-- What a round keeps, and what it does not
-- ---------------------------------------------------------------------------
-- Gone: health, scores, lives, roles, any open vote. Kept: `settings`, because
-- the *rules* did not change - a rematch of a three-life game is a three-life
-- game, and clearing them would let the next join pin different numbers - and
-- the roster, because a rematch an eliminated player has to re-join is a
-- rematch some of the room cannot play.
--
-- ---------------------------------------------------------------------------
-- The one rule here with no real defence, said plainly
-- ---------------------------------------------------------------------------
-- Any player still in the match may call it, and the only check is that the
-- round they name is the next one - which makes two people pressing rematch
-- together idempotent, and does nothing at all about somebody calling it in the
-- middle of a round they are losing.
--
-- That is honest rather than solved. The arbiter has never been told what ends
-- a match: the clock and the score limit are the client's, so there is no
-- moment here that "the match is over" can be checked against. **The fix, when
-- it is worth it, is to tell it** - `join` already pins `hp`, `damage` and
-- `lives`, and a `timeLimit` and `scoreLimit` beside them would let this refuse
-- a reset of a match still being played. It is not built because nothing yet
-- reads a clock server-side, and half of one would be worse than none.
-- ============================================================================
