-- ============================================================================
-- Who may be drawn, which is the only thing about somebody else's deal that
-- ever leaves this table
-- ----------------------------------------------------------------------------
-- docs/xp/xp-flow.md §3. A role has meant exactly one thing since the deck was
-- built - `rules.lethal`, "do this role's shots count" - and the generalisation
-- is two questions: what may you do, and how are you seen.
--
-- **The first needs nothing from here.** A client knows its own secret and
-- nobody else's, and "what may *I* press" is the one question that can be
-- answered from that alone. It is a filter in the runtime and there is no SQL
-- in it.
--
-- **The second cannot be answered by any client, and that is what this is.**
-- Deciding whether to draw somebody needs *that person's* role, and handing a
-- client somebody else's role is the single thing this whole subsystem exists
-- not to do. So the deal carries a second map - value to look - and the party
-- that has already seen the whole deal publishes the *look* per player. Nobody
-- learns a role who did not already hold it.
--
-- What the room does learn is that somebody is invisible, which it could hardly
-- not: there is no body to look at. It learns *which* player, by id, and that
-- is the honest cost of the feature rather than a leak to be closed - a design
-- in which nobody can tell who is missing is a design with no bodies in it at
-- all.
--
-- **`deal` is still the only door, and it is still first-ask-wins.** A client
-- that lied about the looks would have had to lie about the deck in the same
-- call, and that has always been true - so no new trust is placed anywhere.
--
-- Three edits and nothing else: `deal` computes `views` from the map it was
-- given, `reset` clears it with the rest of the round, and the view returns it.
-- The rest of both functions is 20261115000000 unchanged.
-- ============================================================================
