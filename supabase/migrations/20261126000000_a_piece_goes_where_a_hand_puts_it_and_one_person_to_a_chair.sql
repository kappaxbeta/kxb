-- ============================================================================
-- A piece goes where a hand puts it, and one person to a chair
-- ----------------------------------------------------------------------------
-- Two changes to the arbiter, and the first is a repair.
--
-- **20261116000000 was silently undone.** It changed a move from `along` + `to`
-- (a track and a number of squares) to `mark` (the field by name), because
-- nothing computes a move on that board any more - you pick a piece up, carry
-- it, and put it down, and half the places a piece can be put down are not
-- numbered. `20261125000000` then restated the whole function from a base that
-- predates it, so on any database where both ran in order the older shape won.
--
-- Nothing failed loudly: the client asks for `mark`, the arbiter refuses with
-- "a move names the track it is on", and the refusal is swallowed on purpose -
-- a move the arbiter did not take is re-stated by the next one. So a piece moved
-- on the screen of whoever moved it and on no other, which is the exact bug
-- 20261115000000 was written to end. This is why "restate the whole function"
-- needs the *newest* copy as its base, and why this one says so at the top.
--
-- **And seats are claimed rather than dealt.** Which side somebody is on has
-- always been derived - a hash of their id, or a host deciding before anybody
-- loaded the document. A board game wants the third thing: you sit in the chair
-- you picked, and nobody else may sit in it. Three browsers at one table were
-- dealt blue, blue and green by the hash - two people moving the same four
-- pieces, one colour with nobody in it, and no way to fix it from inside the
-- game.
--
-- That half cannot live on a client. *Is blue free* is a race two clients can
-- both answer yes to in the same moment, so Postgres answers it once. What this
-- still does not own is whether the level *has* that side: the document names
-- its own colours and the arbiter has never read one, the same line `roll` and
-- `moved` hold.
...
comment on function public.xp_arbiter_view(text) is
  ...
  'round, whose turn it is and who may be drawn how are public - a look, never '
