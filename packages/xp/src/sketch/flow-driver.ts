...
import { ROUND_AGAIN, RUN_OVER } from '../document/flow'
 ...
 * ---------------------------------------------------------------------------
 * This module moved here from `src/app/xp/_sketch/flow-driver.ts` because
 * the runtime now has two hosts: a web iframe and a phone's WebView. Both
 * drive the same run off the same document, and a host-agnostic pure
 * function belongs where both can reach it - the same argument the contract
 * package makes for `packages/api`. The functions below take numbers and a
 * document and return numbers; nothing here knows about a socket, a DOM, or
 * which host is asking. The import above changed from `@kxb/xp` to a
 * relative path into `../document/flow` - the package's own convention for
 * an internal module reaching another - which is the one deviation from a
 * byte-for-byte move.
 *
 * ---------------------------------------------------------------------------
