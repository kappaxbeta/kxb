...
export function useEditBuffer(slug: string): EditBuffer {
  ...
  const queueRemove = useCallback((position: { x: number; y: number; z: number }) => {
  ...
  useEffect(() => {
    const timer = setInterval(async () => {
      ...
      try {
        const result = await applyLoungeEdits(slug, batch)
        if (result.ok) {
          setError(null)
        } else {
