...
import type { EmoteState } from '@/app/world/_presence/presence-core'
import {
  EMOTE_COLUMNS,
  EMOTE_DURATION_MS,
  EMOTE_ROWS,
 ...
 * React state would re-render every avatar in the room each time anybody in it
...
export function EmoteBubble({
  state,
  /** Clear of the tallest animal in the pack, and above the health bar. */
  height = 3,
}: {
   ...
   * component never suspends. It hangs off an avatar that is already inside a
    ...
    <group ref={group} position={[0, height, 0]} visible={false}>
      <Billboard>
        <mesh userData={{ ignoreRay: true }} renderOrder={20}>
          ...
          {/*
            `depthTest` off for the same reason the health bar turns it off: an
            emote swallowed by the wall somebody is standing behind is an emote
            that did not happen, and being able to see the room react matters
            more here than strict occlusion.
          */}
          <meshBasicMaterial
            ref={material}
            map={texture}
