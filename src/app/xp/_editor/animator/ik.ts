/**
 * Copied from `src/app/ovaloffice/animator/ik.ts`.
 *
...
import type { BoneSpec, Rig } from '@/app/xp/_editor/animator/rig'
 ...
 * move that origin, and the origin is where the handle is. Stops at the rig's
 * root, which is the node the whole document translates and must never be
 * turned by a limb - a wave that quietly rotated the floor under the figure
 * would be a very confusing bug to find.
 *
 * `root` is passed in rather than imported now that there are two rigs. Both
 * happen to call it `root`, which is exactly the coincidence that would have
 * kept a hardcoded name working right up until a third one did not.
 */
export function chainAbove(bone: THREE.Object3D, reach: number, root: string): THREE.Bone[] {
  const chain: THREE.Bone[] = []
  let walk = bone.parent
  while (walk && chain.length < reach && walk.name !== root) {
...
/**
 * Everything a solve needs to know about the body it is solving on.
 *
 * Bundled because the four travel together through every call here and through
 * every call site in `stage.tsx`, and three of them arrived as separate
 * arguments in an order nobody could remember.
 */
export interface Solver {
  rig: Rig
  /** Local rotations as the model shipped - what a hinge is measured against. */
  rest: Map<string, THREE.Quaternion>
  /**
   * A point out along each single-segment part, for the parts that have one.
   *
   * See `BoneSpec.swivel`. A bone dragged by its own origin cannot be pointed
   * anywhere, because the vector from the pivot to the effector is zero; the tip
   * is the effector that makes the same solver work on a one-bone chain.
   */
  tips: Map<string, THREE.Object3D>
}

/**
 * Move one bone to where the pointer put it, whichever kind of bone it is.
 *
 * The one place that knows there are two kinds. It was two branches in
 * `stage.tsx` and a third would have made it three, and `holdPins` below needs
 * exactly the same decision - a pinned peep leg re-solved through `chainAbove`
 * gets an empty chain and silently does nothing, which is the bug this function
 * exists to make unrepresentable.
 */
export function dragBone(
  bone: THREE.Object3D,
  spec: BoneSpec,
  target: THREE.Vector3,
  { rig, rest, tips }: Solver,
  options?: Parameters<typeof reachFor>[5],
): void {
  if (spec.swivel) {
    // One bone, aimed by its own tip. `?? bone` is not a fallback that works -
    // it degenerates to a zero-length effector vector and `reachFor` skips - but
    // it is the honest thing to pass, and a missing tip means the part was not
    // in this animal's file, so there is nothing on screen to have dragged.
    reachFor(tips.get(spec.name) ?? bone, [bone as THREE.Bone], target, rig.specs, rest, options)
    return
  }
  reachFor(bone, chainAbove(bone, spec.reach, rig.root), target, rig.specs, rest, options)
}
...
export function holdPins(
  ...
  for (const [name, at] of pins) {
    ...
    const spec = solver.rig.specs[name]
    if (!bone || !spec) continue
    dragBone(bone, spec, at, solver, { iterations: 8 })
  }
}
