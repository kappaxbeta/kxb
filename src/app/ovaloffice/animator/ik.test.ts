...
import { chainAbove, preBend, reachFor } from '@/app/ovaloffice/animator/ik'
import type { BoneSpec } from '@/domain/animator/rig'
...
describe('chainAbove', () => {
  test('walks up from the parent and stops before the root', () => {
    const { hand } = arm()
    expect(chainAbove(hand, 3).map((bone) => bone.name)).toEqual(['wrist', 'lowerarm', 'upperarm'])
  })

  test('stops at the reach it was given', () => {
    const { hand } = arm()
    expect(chainAbove(hand, 1).map((bone) => bone.name)).toEqual(['wrist'])
  })

  test('never turns the root, however far the reach', () => {
    const { hand } = arm()
    expect(chainAbove(hand, 99).map((bone) => bone.name)).not.toContain('root')
  })
})
...
describe('reaching', () => {
  ...
  test('a two-bone limb cannot fold at all without the pre-bend', () => {
    ...
    const stiff = leg()
    reachFor(stiff.foot, chainAbove(stiff.foot, 2), target, specs, restOf(stiff.bones))
    expect(reached(stiff.foot, target)).toBeGreaterThan(0.2)

    const bent = leg()
    const rest = restOf(bent.bones)
    const chain = chainAbove(bent.foot, 2)
    preBend(chain, specs, rest)
  ...
  test('a target beyond reach straightens the arm towards it rather than breaking', () => {
    ...
    reachFor(hand, chainAbove(hand, 3), target, SPECS, restOf(bones), { iterations: 30 })
  ...
  test('a target sitting exactly on the joint does not produce NaN', () => {
    const { hand, bones } = arm()
    reachFor(hand, chainAbove(hand, 3), new THREE.Vector3(0, 0, 0), SPECS, restOf(bones))
