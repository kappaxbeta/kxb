...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return { title: t.garden, description: t.gardenBody }
}
 ...
 * A member's garden.
 *
 * The same scene as the house, handed a different plan. Yours by default,
 * somebody else's with `?of=<userId>` - see `openPlace`.
 */
export default async function TenantOutdoorPage({
  params,
  searchParams,
}: {
  ...
  const visit = await openPlace(slug, 'outdoor', searchParams)
